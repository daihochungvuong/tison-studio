 ...
 * 
 * Analyze Script大纲，自动识别Main Character的阶段变化，生成多阶段变体。
 * 
 * Feature：
 * 1. 分析大纲中的时间跨度和角色成长轨迹
 * 2. 为主要Character Generation阶段变体（青年版、中年版等）
 * 3. 每变体包含Episodes范围，供Shot时自动调用
 */
...
export interface StageVariationData {
  ...
  stageDescription: string;         // "创业初期，意气风Hair"
  visualPromptEn: string;           // 英文Prompt
...
/**
 * Analyze Script角色，识别需要多阶段形象的角色
 * 
 * @param background 项目背景（包含大纲）
 * @param characters 角色列表
 * @param totalEpisodes 总Episodes
 * @param options API配置
 */
export async function analyzeCharacterStages(
  ...
  // 只分析Main Character（前3或有详细描述的）
  const mainCharacters = characters.slice(0, 5).filter(c => 
    c.role || c.personality || c.appearance
  );
  
  if (mainCharacters.length === 0) {
    console.log('[CharacterStageAnalyzer] 没有找到需要分析的Main Character');
    return [];
  }
  
  const systemPrompt = `你是专业的影Viewpoint色设计顾问，擅长分析角色在长篇剧Episode中的形象变化。

你的任务是Analyze Script大纲，判断每Main Character是否需要多阶段的形象变体。

【判断Standard】
角色需要多阶段形象的情况：
1. 时间跨度大（如从25岁到50岁）
2. 身份地位变化（从普通人到Success企业家）
3. 外貌有显著变化（年轻→成熟→老年）
4. 剧Episodes量多（30Episode以上的主角通常需要）

不需要多阶段的情况：
1. Supporting Character、出场少的角色
2. 时间跨度短的剧Episode
3. 角色外貌None明显变化

【阶段划分原则】
- 根据总Episodes合理划分，每阶段至少10Episode
- 阶段之间要有明显的形象区分
- 保持面部特征、Body type等一致性元素
      ...
      "stages": [
        {
          ...
          "episodeRange": [1, 15],
          "ageDescription": "25岁",
          "stageDescription": "985毕业生，意气风Hair，白衬衫",
${promptLanguage !== 'en' ? '          "visualPromptZh": "25岁中国Male性，干净利落的外表，白色衬衫，自信有抱负的神态"' : ''}${promptLanguage !== 'zh' ? `${promptLanguage === 'zh+en' ? ',' : ''}\n          "visualPromptEn": "25 year old Chinese male, clean-cut appearance, white dress shirt, confident and ambitious look"` : ''}
        },
        {
          "name": "中年版",
          "episodeRange": [16, 40],
...
${promptLanguage !== 'en' ? '          "visualPromptZh": "35-40岁中国Male性，成熟商人形象，剪裁合身的西装"' : ''}${promptLanguage !== 'zh' ? `${promptLanguage === 'zh+en' ? ',' : ''}\n          "visualPromptEn": "35-40 year old Chinese male, mature businessman look, tailored suit, commanding presence"` : ''}
        }
      ],
      "consistencyElements": {
  ...
  try {
    ...
    console.error('[CharacterStageAnalyzer] AI分析Failed:', error);
    return [];
  }
}
...
/**
 * 根据Episodes获取角色应使用的变体
 * 
 * @param variations 角色的变体列表
 * @param episodeIndex 当前Episodes
 * @returns 匹配的变体，如果没有阶段变体则Back undefined
 */
...
export function detectMultiStageHints(outline: string, totalEpisodes: number): {
  ...
  for (const pattern of agePatterns) {
    ...
    if (ageMatch) {
      ...
      if (ageSpan >= 10) { // Age跨度至少10岁
        hasAgeChange = true;
        hints.push(`Age跨度${ageMatch[1]}岁到${ageMatch[2]}岁`);
        break;
      }
    }
  }
  ...
  if (foundKeywords.length > 0) {
    hints.push(`包含阶段Keywords：${foundKeywords.join('、')}`);
  }
  
  // 综合判断 - 降低门槛
  // 1. 20Episode以上且有任何线索
  // 2. 或者40Episode以上的主角剧默认需要多阶段
  const suggestMultiStage = (
    (totalEpisodes >= 20 && (hasTimeSpan || hasAgeChange || foundKeywords.length >= 1)) ||
    (totalEpisodes >= 40) // 40Episode以上的主角剧默认需要
  );
