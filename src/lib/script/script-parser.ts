...
const PARSE_SYSTEM_PROMPT = `你是一专业的Script Analysis师。分析用户提供的剧本/故事文本，提取结构化信息。

请严格按照以下JSON格式Back结果（不要包含任何其他文字）：
{
  "title": "故事标题",
  ...
  "characters": [
    {
      ...
      "gender": "Gender",
      "age": "Age",
      "role": "详细的身份背景描述，包括Occupation、地位、背景故事等",
      ...
      "tags": ["角色标签，如: 武侠, Male主, 剑客, 反派, Female将军"],
      "notes": "角色备注（Plot说明，如: 本剧主角，在第三幕触Hair激烈冲突）"
    }
  ],
  "episodes": [
    {
  ...
  "scenes": [
    {
      "id": "scene_1",
      "episodeId": "ep_1",
      "name": "Scene名称（如：雁城大街、荒野古庙、宫庭内院）",
      "location": "详细Location描述（包括建筑特征、环境元素、地理特点等）",
      "time": "时间设定（day/night/dawn/dusk/noon/midnight）",
      "atmosphere": "详细氛围描述（如：紧张压抑、温馨宁静、神秘阴森、悲壮肇杀）",
      "visualPrompt": "Scene的详细视觉描述，用于生成Scene概念图（包括光线、Weather、建筑风格、特殊元素等，用英文）",
      "tags": ["Scene关键元素标签，如: 木柱, 窗棱, 古建筑, 废墟, 深林"],
      "notes": "Location备注（Plot说明，如: 决战Hair生的古老殿堂）"
    }
  ],
  "storyParagraphs": [
    {
...
重要要求：
1. 【Character Info必须详细】：不要简化Character Info！保留原文中的所有细节：
   ...
   - tags: 角色标签，3-5，描述角色类型和特征（如: 武侠, Male主, 剑客, 守护者）
   - notes: 角色备注，说明这角色在Plot中的作用（如: "本剧主角，第三幕触Hair冲突"）
2. 【Scene设计必须详细】：不要简化Scene信息！Scene是视觉生成的基础：
   - name: Scene名称要具体有辨识度（不要只写"Indoor""Outdoor"）
   - location: 详细Location描述，包括建筑特征、环境元素
   - time: 使用英文时间词（day/night/dawn/dusk/noon/midnight）
   - atmosphere: 详细氛围，不要只写一字
   - visualPrompt: 用英文写出Scene的视觉描述（光线、Weather、风格、建筑特征等），例如：
   ...
   - tags: Scene关键元素标签，3-6，描述环境特征（如: 木柱, 窗棱, 古建筑, 烟雾, 残垣断壁）
   - notes: Location备注，说明这Scene在Plot中的作用（如: "决战Hair生的古老殿堂"）
3. 识别多Episode结构。如果剧本包含"第XEpisode"、"Episode X"、"第X章"等标记，要拆分成多 episode
4. 如果没有明确的Episode标记，Create单 episode 包含所有Scene
5. 角色ID使用 char_1, char_2 格式
6. SceneID使用 scene_1, scene_2 格式
7. EpisodeID使用 ep_1, ep_2 格式`;

// Per-scene shot generation prompt (based on CineGen-AI)
const SHOT_GENERATION_SYSTEM_PROMPT = `你是一专业的Shot师/摄影指导。为单Scene生成电影级别的详细Shot列表（Camera Blocking）。

请严格按照以下JSON数组格式Back结果（不要包含任何其他文字）：
[
  {
    "sceneId": "scene_1",
    "shotSize": "景别（WS/MS/CU/ECU）",
    "duration": 4.0,
    "visualDescription": "详细的中文Visual Description，包括Scene、光线、角色动作、表情等",
    "actionSummary": "简短的动作概述",
    "cameraMovement": "Shot运动",
    "dialogue": "对白内容（包含说话者和语气）",
    "ambientSound": "环境声描述",
    "soundEffect": "Sound Effect描述",
    "characters": ["角色名"],
    "keyframes": [
      {
        "id": "kf-1-start",
        "type": "start",
        "visualPrompt": "详细的英文视觉描述（用于Image Generation）"
      }
    ]
  }
]
...
Shot原则：
1. 【重要】每Scene最多6-8Shot，避免JSON截断
2. 【景别缩写】WS=Long Shot, MS=Medium Shot, CU=Close Shot, ECU=Close-up, FS=Wide Shot
3. 【Shot运动】使用专业术语：
...
4. 【视觉描述】visualDescription 要像写电影文学剧本，详细描述：
   - Scene光影（如"黑暗中微弱光芒笼罩"）
   - Character Status（如"身穿明黄色八卦袝，身姿矫健"）
   - 气氛营造（如"紧张的对峰气氛"）
   - 具体动作（如"Shot缓缓推进"）
5. 【Audio设计】每Shot都要考虑：
   - ambientSound: Ambient Sound（风声、雨声、人声鼎沸、寢静等）
   - soundEffect: Sound Effect（脚步声、剑鸣、门响、爆炸等）
   - dialogue: 对白要包含Speaker和语气（如"天师（低沉肇立）：天地大None边..."）
6. 【Duration】duration 估算每Shotsec数（2-8sec，根据内容复杂度）
7. 【visualPrompt】英文描述，40词内，用于Image Generation，格式：
...
interface ParseOptions {
  ...
  model: string;
  language?: string;
  sceneCount?: number; // 限制Scene数量（用于预告片等）
  shotCount?: number; // 每SceneShot数Notice（传递给后续 shot generation）
  keyManager?: ApiKeyManager; // Optional: use existing key manager for rotation
  temperature?: number; // Custom温度，默认 0.7
  maxTokens?: number; // Custom最大输出 token 数，默认 4096
  /** Close推理Model深度思考（智谱 GLM-4.7/4.5 等），避免 reasoning 耗尽 token */
...
interface ShotGenerationOptions extends ParseOptions {
  ...
  shotCount?: number; // 限制Total Shots量（用于预告片等）
  concurrency?: number; // 并行处理Scene数（默认1，多 key 时可设置更高）
}
...
export async function callChatAPI(
  ...
  console.log(`[callChatAPI] 使用 ${provider}，Total ${totalKeys}  API keys`);

  if (!baseUrl) {
    throw new Error('Base URL Not Configured');
  }
  if (!model) {
    throw new Error('ModelNot Configured');
  }
  ...
  // 从 Model Registry 查询Model限制（三层查找：缓存→静态→default）
  ...
  return await retryOperation(async () => {
    ...
    // ModelSelect逻辑：必须使用配置 model
    const modelName = model;
    console.log('[callChatAPI] 使用Model:', modelName);
    ...
    // 智谱推理Model (GLM-4.7/4.5 等) 支持通过 thinking.type Close深度思考
    if (options.disableThinking) {
      body.thinking = { type: 'disabled' };
      console.log('[callChatAPI] 已Close深度思考 (thinking: disabled)');
    }
      ...
      if (response.status === 400) {
        ...
        if (discovered) {
          ...
          if (discovered.maxOutput && effectiveMaxTokens > discovered.maxOutput) {
            ...
            console.warn(
              `[callChatAPI] 🧠 Hair现 ${model} maxOutput=${discovered.maxOutput}，` +
            ...
            if (retryResp.ok) {
              ...
              console.warn('[callChatAPI] Hair现重试仍Failed:', retryResp.status);
            }
          }
        }
      }
    ...
    if (!content) {
      // 诊断日志：记录 API 实际Back的结构
      const finishReason = data.choices?.[0]?.finish_reason;
      const usage = data.usage;
      const reasoningContent = data.choices?.[0]?.message?.reasoning_content;
      console.error('[callChatAPI] ⚠️ API Back空内容！诊断信息:');
      ...
      // 推理Model回退：如果有 reasoning_content 但 content 为空，说明Model耗尽 token 在思考上
      if (finishReason === 'length' && reasoningContent) {
        ...
        // 说明Model在「思考」上花了太多预算，以双倍 max_tokens 自动重试一次
        ...
        if (reasoningTokens > 0 && completionTokens > 0 &&
          ...
          console.warn(
            `[callChatAPI] 推理Model token 耗尽 (reasoning: ${reasoningTokens}/${completionTokens})，` +
          ...
          if (retryResp.ok) {
            ...
            console.warn('[callChatAPI] 重试请求Failed:', retryResp.status);
          }
        } else {
          console.warn(
            `[callChatAPI] 推理Model token 耗尽：reasoning ${reasoningContent.length} 字，content 为空。` +
...
export async function parseScript(
  ...
  const sceneCountHint = options.sceneCount 
    ? `\n\n【重要】请仅提取最重要的 ${options.sceneCount} Scene，挑选Plot中最具代表性和视觉冲击力的Scene。`
    : '';
...
语言：${options.language || '中文'}${sceneCountHint}`;
  ...
  try {
    ...
    const scenes = (parsed.scenes || []).map((s: any, i: number) => ({
      ...
      location: s.location || '未知Location',
      time: normalizeTimeValue(s.time),
      atmosphere: s.atmosphere || '',
      visualPrompt: s.visualPrompt || '', // 用于Scene概念图生成
      tags: s.tags || [],        // Scene标签
      notes: s.notes || '',      // Scene备注
    ...
    throw new Error('None法解析AIBack的剧本数据');
  }
}
...
export async function generateShotList(
  ...
  onShotsGenerated?: (newShots: Shot[], sceneIndex: number) => void // 流式回调，每SceneDone后立即通知
): Promise<Shot[]> {
  ...
  if (targetShotCount) {
    ...
    // 根据Duration计算合理的每SceneShot数（参考：每Shot约2-5sec）
    const totalBudget = Math.max(2, Math.ceil(durationSec / 3));
    shotsPerScene = Math.max(1, Math.ceil(totalBudget / totalScenes));
    shotsPerSceneHint = `${shotsPerScene}（目标Duration ${durationSec}sec，总计约 ${totalBudget} Shot）`;
  }
  ...
  const processScene = async (sceneIndex: number): Promise<Shot[]> => {
    ...
    const sceneContent = paragraphs.trim() 
      ? paragraphs 
      : `Scene${sceneIndex + 1}: ${scene.name || scene.location}，${scene.atmosphere || ''}环境`;

    const userPrompt = `为Scene ${sceneIndex + 1} 生成电影级别的详细Shot。
输出语言: ${lang}

=== Scene信息 ===
Scene名: ${scene.name || scene.location}
Location: ${scene.location}
时间: ${scene.time}
氛围: ${scene.atmosphere}
${(scene as any).visualPrompt ? `Scene视觉参考: ${(scene as any).visualPrompt}` : ''}
...
目标Duration: ${options.targetDuration}
...
=== Shot要求 ===
1. 为该Scene生成${shotsPerScene ? `恰好 ${shotsPerScene} ` : shotsPerSceneHint}Shot，挑选最具视觉冲击力的画面
2. 每Shot必须包含：
   - shotSize: 景别（WS/MS/CU/ECU）
   - duration: Duration（sec）
   - visualDescription: 详细中文Visual Description（像写电影剧本那样详细）
   - actionSummary: 简短动作概述
   - cameraMovement: Shot运动
   - ambientSound: 环境声
   - soundEffect: Sound Effect
   - dialogue: 对白（包含Speaker和语气）
   - characters: 出场角色名列表
   - keyframes: 包含start关键帧的visualPrompt（英文，40词内）
3. visualDescription 要详细，包括光影、Character Status、气氛、Shot运动
4. Audio设计要具体，能复现Scene氛围`;
 ...
 */
// 基础 prompt（用于NoneShot结构的创意输入：MV、广告、一句话创意等）
const CREATIVE_SCRIPT_BASE_PROMPT = `你是一位专业的影视编剧和Shot师。根据用户的创意输入，生成完整的剧本。
...
Output Format必须严格遵循（这是Import系统的Standard格式）：
...
**人物小传：**
角色A：[XX岁]，[身份/Occupation]，[性格特点]，[外貌特征]
角色B：[XX岁]，[身份/Occupation]，[性格特点]，[外貌特征]
...
角色A：（动作/表情）Dialogue内容

角色B：（动作/表情）Dialogue内容
...
重要要求：
1. 必须包含《标题》、**大纲：**、**人物小传：**、**第XEpisode**
2. Scene头格式：**编号 日/夜 内/外 Location**
3. 每Scene必须有"人物："行
4. 动作描写用 △ 开头
5. 对白格式：角色名：（动作）Dialogue
6. MV/广告也要拆分成Scene和Shot，只是内容侧重画面和Sound Effect
7. 语言与用户输入保持一致（中文输入用中文输出）
8. **时代一致性**：大纲中必须明确时代背景；人物小传中的Costume、Hair型、道具必须严格符合该时代（如古代剧不得出现现代Costume/电子产品；现代剧不得出现古代服饰）
9. **世界观一致性**：SceneLocation、建筑风格、社会规则必须符合剧本设定的世界观，不得出现矛盾元素`;

// 针对已有Shot结构输入的额外指令（如【Shot1】到【镜夷12】）
const STORYBOARD_STRUCTURE_PROMPT = `

**★★★ 检测到已有Shot结构，必须遵守以下规则 ★★★**

1. 保留原有的每一Shot/Scene，一都不能少
2. 用户输入有12Shot，输出必须有12Scene
3. 每原始Shot转换为一 **X-X 日/夜 内/外 Location** 格式的Scene
4. 绝对禁止合并、省略、压缩Shot数量

**★★★ Scene内容格式（极其重要）★★★**

每Scene内只能有：
1. 人物行：人物：角色A、角色B
2. 一动作行：△[将该Shot所有画面、动作、对白、Sound Effect等压缩为一句完整的视觉描述]

禁止在Scene内写多行！禁止分别列出对白、Sound Effect！所有内容必须压缩到一 △ 行中。

示例：
用户输入【Shot1】包含Visual Description+对白+Sound Effect，你的输出应该是：
**1-1 日 内 篮球馆**
人物：马一花、沈星晴
△记分牌Close-up显示68:70，马一花带球被包夹表情焦躁，全场屏息，心跳声逐渐响起
...
export async function generateScriptFromIdea(
  ...
  // 根据Duration生成参考范围（不是硬限制，是给 AI 的参考）
  ...
  if (durationSeconds > 0 && !sceneCount && !shotCount) {
    ...
    durationGuidance = `\n- Duration参考：${durationSeconds}secVideo通常包含 ${minShots}-${maxShots} Shot，请根据内容需要自行把握节奏`;
  }

  // 检测输入类型
  const inputType = detectInputType(idea);
  
  // 统计原始输入中的Shot/Scene数量
  // 支持多种格式：【Shot1】、**【Shot1：...】**、Shot1、Scene1 等
  ...
  console.log('[generateScriptFromIdea] Shot匹配:', shotMatches);
  console.log('[generateScriptFromIdea] Scene匹配:', sceneMatches);
  
  // 如果检测到已有Shot结构，强调保留
  const preserveStructureNote = originalShotCount > 0 
    ? `\n\n**★★★ 特别注意 ★★★**
用户输入包含 ${originalShotCount} Shot/Scene，你的输出必须有对应的 ${originalShotCount} Scene（**1-1** 到 **1-${originalShotCount}**）。

重要：每Scene内只能有一 △ 动作行！将该Shot的所有画面、对白、Sound Effect压缩成一句话。
禁止分别列出多行对白或Sound Effect，否则会生成多Shot！`
    : '';
...
[要求]
- 语言：${language}
- 目标Duration：${targetDuration === 'auto' ? '根据内容自行决定' : `约 ${targetDuration}`}${durationGuidance}
${originalShotCount > 0 ? `- Scene数量：必须有 ${originalShotCount} （与原始Shot一一对应）` : sceneCount ? `- Scene数量：约 ${sceneCount} ` : '- Scene数量：根据内容和Duration自行决定'}
${originalShotCount > 0 ? '' : shotCount ? `- Shot数量：约 ${shotCount} ` : '- Shot数量：根据内容和Duration自行决定'}
${styleId ? `- Visual Style：${styleId}` : ''}

请生成符合Standard格式的完整剧本，包含：
1. 剧本标题
2. 大纲（简述主题/故事）
3. 人物小传（每角色的基本信息）
4. 完整的Scene和对白${preserveStructureNote}`;

  console.log('[generateScriptFromIdea] 输入类型:', inputType);
  console.log('[generateScriptFromIdea] 创意内容:', idea.substring(0, 100));
  console.log('[generateScriptFromIdea] 检测到原始Shot数:', originalShotCount);
  
  // 根据是否有Shot结构Select不同的 system prompt
  // - 有Shot结构：使用基础 + Shot结构特殊指令（每Scene只能有一动作行）
  // - NoneShot结构：使用基础 prompt（允许正常展开多动作/对白）
  const systemPrompt = originalShotCount > 0
    ? CREATIVE_SCRIPT_BASE_PROMPT + STORYBOARD_STRUCTURE_PROMPT
    : CREATIVE_SCRIPT_BASE_PROMPT;
  
  console.log('[generateScriptFromIdea] 使用 prompt 类型:', originalShotCount > 0 ? 'Shot结构模式' : '普通创意模式');
  
  // 对于详细Shot脚本，需要更高的 max_tokens
  const extendedOptions = {
    ...options,
    maxTokens: originalShotCount > 5 ? 8192 : 4096, // 多Shot时增加输出长度
  };
...
function detectInputType(input: string): string {
  ...
  // 检测已有Shot结构：【ShotX】或 **【ShotX】**
  if (/[【\[]\s*Shot\s*\d+/i.test(trimmed) || /\*\*.*Shot.*\*\*/i.test(trimmed)) {
    return '详细Shot脚本';
  }
  ...
  // 检测广告简报
  if (/广告|宣传[片Video]|commercial|ad\s*brief|品牌/i.test(trimmed)) {
    return '广告简报';
  }
  
  // 检测预告片
  if (/预告[片Video]|trailer|宣传片/i.test(trimmed)) {
    return '预告片脚本';
  }
  
  // 检测短Video
  if (/短Video|抹音|tiktok|快手|reels/i.test(trimmed)) {
    return '短Video创意';
  }
