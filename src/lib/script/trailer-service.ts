...
/**
 * Trailer Service - AI Trailer Shot挑选服务
 * 
 * Feature：从已有的Shot中智能挑选关键Shot，生成预告片
 * 挑选Standard：
 * - 叙事Feature为"高潮/转折"的优先
 * - 有强烈Mood标签的优先
 * - 有视觉冲击的Scene优先
 * - 关键角色出场的优先
 */
 ...
 * AI 挑选Trailer Shot
 * 
 * @param shots 所有可用的Shot
 * @param background 项目背景信息
 * @param duration 预告片Duration
...
export async function selectTrailerShots(
  ...
  try {
    ...
    const systemPrompt = `你是一位专业的电影预告片剪辑师，擅长从大量素材中挑选最具吸引力的Shot来制作预告片。

你的任务是从给定的Shot List中挑选出最适合做预告片的 ${targetCount} Shot。

【预告片结构原则】
1. **开场**：建立氛围，吸引注意（1-2Shot）
2. **冲突升级**：展示故事的核心冲突（2-4Shot）
3. **高潮悬念**：最具张力的画面，留下悬念（1-2Shot）

【挑选Standard】
- 优先Select叙事Feature为"高潮"、"转折"、"冲突"的Shot
- 优先Select有强烈Mood（tense, excited, mysterious）的Shot
- 优先Select有视觉冲击力的画面（动作场面、Close-up、对峙）
- 优先SelectMain Character出场的关键时刻
- 覆盖不同Episodes，展示故事跨度
- 避免剧透关键结局

【输出要求】
请Back一 JSON 数组，包含你挑选的ShotIndex（index），按预告片播放顺序排列。
...
【Shot List】（Total ${shots.length} Shot）
${shotSummaries.map(s => 
  `[${s.index}] ${s.id}
   动作：${s.actionSummary.slice(0, 100)}
   描述：${s.visualDescription.slice(0, 100)}
   角色：${s.characterNames.join('、') || 'None'}
   叙事Feature：${s.narrativeFunction || '未知'}
   Mood：${Array.isArray(s.emotionTags) ? s.emotionTags.join(', ') : 'None'}`
).join('\n\n')}

请从以上Shot中挑选 ${targetCount} 最适合做预告片的Shot，Back JSON 格式的Index列表。`;

    // 统一从服务映射获取配置
    const result = await callFeatureAPI('script_analysis', systemPrompt, userPrompt);

    // 解析 AI Back的 JSON - 支持多种格式
    ...
    // 如果上面Failed，尝试直接匹配数Word Count组 [1, 2, 3, ...]
    ...
    if (selectedIndices.length === 0) {
      throw new Error('AI Back格式Error，None法解析Index');
    }
    ...
    return {
      ...
      error: 'AI 挑选Failed，使用规则挑选',
    };
  }
}

/**
 * 规则挑选（AI Failed时的回退方案）
 */
...
/**
 * 将挑选的 Shot 转换为 SplitScene 格式（用于 AI 导演ShotEdit）
 */
export function convertShotsToSplitScenes(
  ...
  return shots.map((shot, index) => ({
    ...
    // Seedance 1.5 Pro 要求 4-12 sec，强制限制范围
