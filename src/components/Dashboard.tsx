...
export function Dashboard() {
  ...
  const handleDuplicate = useCallback(async (projectId: string) => {
    ...
    try {
      ...
      if (!fs) {
        toast.warning('File storage is not available, only copied project name');
      ...
      const newProjectName = `${source.name} (Copy)`;
      ...
      if (copiedCount > 0) {
        toast.success(`Duplicated project "${source.name}" (${copiedCount} data files)`);
      } else {
        toast.warning('No project data found, only project name was copied');
      }
      ...
      toast.error(`Failed to duplicate project data: ${(err as Error).message}`);
    } finally {
      setDuplicatingId(null);
    }
  }, [projects]);
    ...
    <div className="flex flex-col h-full bg-background overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-foreground mb-1">My Projects</h2>
      ...
      <Dialog open={renameDialogOpen} onOpenChange={setRenameDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRename} disabled={!renameValue.trim()}>Confirm</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={batchDeleteConfirm} onOpenChange={setBatchDeleteConfirm}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            About to delete <span className="text-foreground font-medium">{selectedIds.size}</span> project(s).
            This action cannot be undone. Proceed?
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBatchDeleteConfirm(false)}>Cancel</Button>
            <Button variant="destructive" onClick={handleBatchDelete}>Confirm Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
