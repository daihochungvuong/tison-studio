...
/**
 * use-sclass-generation.ts — S级 Seedance 2.0 Video Generation Hook
 *
 * 核心Feature：
 * 1. generateGroupVideo(group) — 单组生成：收Episode @引用 → 构建多模态请求 → 调用 API → 轮询
 * 2. generateAllGroups() — Batch Generate：逐组串行，各组独立生成
 * 3. generateSingleShot(sceneId) — 单镜生成（兼容模式）
 * 4. 自动Upload base64/local Image到 HTTP URL
...
export function useSClassGeneration() {
  ...
  const generateGroupVideo = useCallback(
    async (
      ...
      options?: {
        ...
        /** 构建完格子图+prompt 后，询问用户是否继续Generate Video；Back false 则中止 */
      ...
      if (!keyManager.getCurrentKey()) {
        return {
          ...
          success: false,
          videoUrl: null,
          error: "Please first在设置中配置Video Generation API Key",
        };
      }
      const sclassProjectData = getProjectData(projectId);
      const sclassConfig = sclassProjectData.config;

      // 1b. 从 director-store 直读Total享配置（单一数据源，避免双 store 同步问题）
      ...
      // 4. 从组内Shot聚合Audio/运镜设置
        ...
        if (!isExtendOrEdit) {
          ...
          // 收Episode组内Shot的First FrameImage
          ...
          if (firstFrameUrls.length > 0) {
            ...
            if (canReuseGrid) {
              ...
              console.log('[SClassGen] 已合并', firstFrameUrls.length, '张First Frame为格子图');
            }
        ...
        // 5. 收EpisodeImage引用 → 转 HTTP URL
        const imageRefs = promptResult.refs.images;
        const imageWithRoles = await prepareImageUrls(imageRefs);

        // 5b. 收EpisodeVideo/Audio引用 → 转 HTTP URL（Seedance 2.0 多模态输入）
        const videoRefUrls: string[] = [];
        // 前组Video衔接（链式重试时传入）— 延长/Edit组已在 refs.videos 中携带 sourceVideoUrl，跳过
        ...
        for (let attempt = 0; attempt < maxVideoAttempts; attempt++) {
          ...
          try {
            ...
            const fallbackStatus = /model|Model/i.test(message)
              && /not support|unsupported|None权限|权限不足|未开通|不可用/i.test(message)
              ? 400
              : undefined;
            ...
            const retryableByMessage = /429|500|502|503|529|too many requests|rate|quota|service unavailable|overloaded|internal server error|server error|上游负载|上游服务|饱和|暂时不可用|服务暂时不可用|api key|None效|过期|model|Model|不支持|权限|未开通/.test(message.toLowerCase());
        ...
        if (!videoUrl) {
          throw lastVideoError || new Error("VideoGeneration failed：没有可用 API Key");
        }
        ...
        const errorMsg = err.message || "VideoGeneration failed";
  ...
  // ========== Batch Generate（逐组串行 + Tail Frame传递） ==========
  ...
  const generateAllGroups = useCallback(
    async (
      ...
      if (groupsToGenerate.length === 0) {
        toast.info("所有Shot组Generated或正在Generating...");
        return [];
      }
      ...
      toast.info(
        `开始逐组生成 ${groupsToGenerate.length} Shot组Video...`
      );

      for (let i = 0; i < groupsToGenerate.length; i++) {
        if (abortRef.current) {
          toast.warning("已中止Batch Generate");
          break;
        }
        ...
        if (result.success) {
          toast.success(
            `组 ${i + 1}/${groupsToGenerate.length} 「${group.name}」生成Done`
          );
        } else {
          toast.error(
            `组 ${i + 1}/${groupsToGenerate.length} 「${group.name}」Failed: ${result.error}`
          );
        }
      }
      ...
      if (failCount === 0) {
        toast.success(`全部 ${successCount} Shot组生成Done 🎬`);
      } else {
        toast.warning(
          `生成完毕：${successCount} Success，${failCount} Failed`
        );
      }
  ...
  const generateSingleShot = useCallback(
    async (sceneId: number): Promise<boolean> => {
      ...
      const keyManager = featureConfig.keyManager;
      if (!keyManager.getCurrentKey()) {
        toast.error("Please first在设置中配置Video Generation API Key");
        return false;
      }
      const projectId = activeProjectId;
      if (!projectId) return false;

      // 从 director-store 直读Total享配置（与 generateGroupVideo 保持一致）
      ...
      try {
        ...
        toast.success(`Shot ${sceneId + 1} 生成Done`);
        return true;
      } catch (error) {
        const err = error as Error;
        updateSingleShotVideo(sceneId, {
        ...
        toast.error(`Shot ${sceneId + 1} generation failed: ${err.message}`);
        return false;
      }
    },
    [
  ...
  /**
   * 基于已Done组Create延长子组并Generate Video
   *
   * @param sourceGroupId 来源组 ID（必须已Done且有 videoUrl）
   * @param extendDuration 延长Duration (4-15s)
  ...
  const generateChainExtension = useCallback(
    async (
      ...
      if (!sourceGroup || !sourceGroup.videoUrl) {
        toast.error('源组None已DoneVideo，None法延长');
        return null;
      }
      ...
      toast.info(`已Create延长子组「${childGroup.name}」`);
