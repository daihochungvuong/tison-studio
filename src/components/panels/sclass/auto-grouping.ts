 ...
 * 策略：
 *   1. 按顺序贪心填装，每组总Duration ≤ maxDuration（默认15s）
 *   2. Scene切换优先断开（不同 sceneName 的Shot优先不在同一组）
 *   3. 角色重叠度高的Shot优先同组（characterIds 交Episode）
 *   4. 每组 2~maxPerGroup Shot
 */
...
export interface GroupingConfig {
  /** 单组最大Duration（sec），默认 15 */
  maxDuration: number;
  /** 单组最大Shot数，默认 4 */
  maxPerGroup: number;
  /** 单组最小Shot数，默认 1（最后一组可能为 1） */
  minPerGroup: number;
  /** 默认单镜Duration（当 scene.duration 未设置时），默认 5 */
...
export function generateGroupName(
  ...
  if (group.sceneIds.length === 0) return `Group ${groupIndex + 1}`;
  ...
  return `Group ${groupIndex + 1}: Shot${firstNum}-${lastNum}`;
}
