...
export function ExportPanel({ projectName, scriptData, shots, targetDuration }: ExportPanelProps) {
  ...
  const handleExportToFolder = async () => {
    ...
    try {
      ...
      toast.error(`ExportFailed: ${err.message}`);
    } finally {
  ...
  const handleDownloadFiles = async () => {
    ...
    try {
      ...
      const err = error as Error;
      toast.error(`DownloadFailed: ${err.message}`);
    } finally {
      setIsExporting(false);
      setProgress(null);
    }
  ...
  return (
    <div className="space-y-4 p-4 rounded-lg border bg-card">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-sm">Export Asset Package</h3>
        <span className="text-xs text-muted-foreground">
          可用于剪映、PR等Video Edit软件
        </span>
      </div>
      ...
      <div className="text-xs text-muted-foreground space-y-1">
        <div className="flex items-center gap-1">
          ...
          Includes manifest.json metadata file
        </div>
        <div>Folder结构: images/, videos/, manifest.json</div>
      </div>
      ...
      {!stats.canExport && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          Please firstGenerating shotImage或Video
        </div>
      )}
    </div>
  );
