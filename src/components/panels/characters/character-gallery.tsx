...
export function CharacterGallery({ onCharacterSelect, selectedCharacterId }: CharacterGalleryProps) {
  ...
  const visibleCharacters = useMemo(() => {
    ...
    // Current Episode过滤：只显示Current Episode关联的角色 + NoneEpisodeBinding的全局角色
  ...
  const handleDeleteFolder = (id: string) => {
    if (confirm("Confirm delete this folder? Characters inside will be moved to the parent directory.")) {
      deleteFolder(id);
  ...
  const handleDeleteCharacter = (char: Character) => {
    if (confirm(`Confirm delete character "${char.name}" ?`)) {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-3 pb-2 border-b space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Full Series / Current Episode Toggle (Only shown when entering an episode)*/}
          {hasEpisodeScope && (
                      ...
                      <>
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {char.views.length > 0 ? `${char.views.length} views` : "Not generated"}
                          </p>
                        </div>
                      </>
                    ) : (
          ...
          subFolders.length === 0 && (
            <div className="flex flex-col items-center justify-center h-[200px] text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Use the left console to create characters
              </p>
            </div>
          )
        )}
