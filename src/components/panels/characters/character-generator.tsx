...
export function CharacterGenerator({ character }: CharacterGeneratorProps) {
  ...
  const handleSaveDescription = () => {
    if (description.trim() !== character.description) {
      ...
      toast.success("DescriptionSaved");
    }
  };
  ...
  const handleSavePreview = async () => {
    ...
    try {
      ...
      // 同步归档到Media Library AIImage Folder
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="relative rounded-lg overflow-hidden border-2 border-amber-500/50 bg-muted">
          <img 
            src={previewUrl} 
            alt={`${character.name} Character design preview`}
        ...
        {previewPrompt && (
          <details className="text-xs text-muted-foreground">
            <summary className="cursor-pointer hover:text-foreground">View Generated Prompt</summary>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-sm">Generate Character Design Sheet</h3>
        {isGenerating && (
      ...
      {existingSheet && (
        <div className="relative rounded-lg overflow-hidden border bg-muted">
          <img 
            ...
            alt={`${character.name} Character Design Sheet`}
            className="w-full h-auto"
          />
      ...
      <div className="space-y-2">
        <Label className="text-xs">Character Description(For AI Generation)</Label>
        <Textarea
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          onBlur={handleSaveDescription}
          placeholder="Describe character appearance in detail..."
      ...
      <div className="space-y-2">
        <Label className="text-xs">Design Sheet Content</Label>
        <div className="space-y-2">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {element.id === 'three-view' && 'Front, side, back 3-view structure'}
                  {element.id === 'expressions' && 'Various facial expressions'}
                  {element.id === 'proportions' && 'Body proportions, head-to-body ratio reference'}
                  {element.id === 'poses' && 'Various common action poses'}
                </p>
              </div>
            </div>
          ))}
      ...
      >
        {isGenerating ? (
          <>
            ...
            Generating...Character Design Sheet图...
          </>
        ) : (
          <>
            <FileImage className="h-4 w-4 mr-2" />
            {existingSheet ? 'Regenerate Design Sheet' : 'Generate Character Design Sheet'}
          </>
        )}
      </Button>
      ...
      {character.referenceImages && character.referenceImages.length > 0 && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            AI将参考这些Image GenerationCharacter Design Sheet图
          </p>
        </div>
      )}
      ...
      <div className="text-xs text-muted-foreground space-y-1">
        <p>💡 After generation, you can preview and confirm. Save if satisfied.</p>
        <p>💡 Saved characters can be dragged to AI Director panel</p>
      </div>
    </div>
  );
}
