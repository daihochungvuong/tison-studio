...
export function ShotGridView({ onGenerateImage, onGenerateVideo }: ShotGridViewProps) {
  ...
  const handleGenerateKeyframe = async (shot: Shot, type: "start" | "end") => {
    if (!onGenerateImage) {
      toast.error("Image Gen service not configured");
      return;
    }
  ...
  const handleGenerateVideo = async (shot: Shot) => {
    if (!onGenerateVideo) {
      toast.error("Video Gen service not configured");
      return;
    }
  ...
  const handleBatchGenerateImages = async () => {
    ...
    if (allStartFramesGenerated) {
      if (!confirm("Confirm要Regenerate所Has Shot的First Frame?")) return;
    }
    ...
    setBatchProgress({
      ...
      message: allStartFramesGenerated ? "Regenerating all first frames..." : "Batch generating first frames...",
    });
    ...
    toast.success("Batch GenerateDone");
  };
  ...
  if (shots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-zinc-500 bg-[#121212] p-8">
        ...
        <p className="text-sm">NoneShot Count据</p>
        <p className="text-xs text-zinc-600 mt-1">Please first在Script阶段Generating shot List</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#121212] relative overflow-hidden">
      ...
      <BatchProgressOverlay
        ...
        title="Batch GenerateFirst Frame"
      />
      ...
      <div className="h-14 border-b border-zinc-800 bg-[#1A1A1A] px-4 flex items-center justify-between shrink-0">
        ...
        >
          ...
          {allStartFramesGenerated ? "RegenerateFirst Frame" : "Batch GenerateFirst Frame"}
        </Button>
      </div>
              ...
              <div className="space-y-5">
                ...
                {activeScene && (
                  <div className="bg-[#141414] p-4 rounded-lg border border-zinc-800 space-y-3">
                    <div className="flex items-center gap-2 text-zinc-400">
                      ...
                      <span className="text-xs font-medium uppercase tracking-wider">Scene Env</span>
                    </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-zinc-400">
                    ...
                    <span className="text-xs font-medium uppercase tracking-wider">Visual Prod</span>
                  </div>
                ...
                <div className="bg-[#141414] rounded-lg p-4 border border-zinc-800 space-y-3">
                  ...
                  >
                    {processingType === "video" ? (
                      ...
                      <>
                        {activeShot.videoUrl || activeShot.interval?.videoUrl ? "RegenerateVideo" : "Generate Video"}
                      </>
                    )}
                  </Button>
                  ...
                  {!endKf?.imageUrl && (
                    <p className="text-[9px] text-zinc-600 text-center">
                      * Not setEnd Frame，Will use Single Image mode (I2V)
                    </p>
                  )}
                </div>
              </div>
