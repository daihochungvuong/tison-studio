...
export async function convertToHttpUrl(
  ...
  if (!isImageHostConfigured()) {
    throw new Error('图床Not Configured，请在设置中配置图床 API Key');
  }

  let imageData = url;
  if (url.startsWith('local-image://')) {
    const base64 = await readImageAsBase64(url);
    if (!base64) throw new Error(`None法读取本地文件: ${url.substring(0, 40)}`);
    imageData = base64;
  }
  ...
  if (!result.success || !result.url) {
    throw new Error(result.error || '图床UploadFailed');
  }
  return result.url;
}
...
/**
 * Gemini supported_endpoint_types → 内部Video路由格式
 * 基于 /api/pricing_new Back的元数据，而非Model名猜测
 */
const VIDEO_FORMAT_MAP: Record<string, 'openai_official' | 'unified' | 'volc' | 'wan' | 'kling' | 'replicate'> = {
  ...
  'openai': 'unified', // 某些CustomProvider会把VideoModel标记为通用 openai
  'Video统一格式': 'unified',
  'grokVideo': 'unified',
  'openai-response': 'unified',
  '海螺Video Generation': 'unified',
  'lumaVideo Generation': 'unified',
  'lumaVideo扩展': 'unified',
  'runway图生Video': 'unified',
  'aigc-video': 'unified',
  'wanVideo Generation': 'unified',
  // Vidu (all route to unified /v1/video/generations)
  'vidu文生Video': 'unified',
  'vidu图生Video': 'unified',
  'vidu参考生Video': 'unified',
  'vidu首Tail Frame': 'unified',
  'lumaVideo延长': 'unified',
};

/**
 * 统一格式端点路径映射（端点类型 → Submit/轮询 URL 路径）
 * 每种端点类型直接对应Confirm的 URL，不再靠 fallback 猜测
 */
const UNIFIED_ENDPOINT_PATHS: Record<string, { submit: string; poll: (id: string) => string }> = {
  ...
  '海螺Video Generation': { submit: '/minimax/v1/video_generation', poll: (id) => `/minimax/v1/query/video_generation?task_id=${id}` },
  'lumaVideo Generation': { submit: '/luma/generations',            poll: (id) => `/luma/generations/${id}` },
  'lumaVideo扩展': { submit: '/luma/generations',            poll: (id) => `/luma/generations/${id}` },
  'lumaVideo延长': { submit: '/luma/generations',            poll: (id) => `/luma/generations/${id}` },
  'runway图生Video': { submit: '/runwayml/v1/image_to_video', poll: (id) => `/runwayml/v1/tasks/${id}` },
  'wanVideo Generation':    { submit: '/alibailian/api/v1/services/aigc/video-generation/video-synthesis', poll: (id) => `/alibailian/api/v1/tasks/${id}` },
  ...
  'vidu首Tail Frame':     { submit: '/ent/v2/start-end2video',  poll: (id) => `/ent/v2/task?task_id=${id}` },
};
const DEFAULT_UNIFIED_ENDPOINT = { submit: '/v1/video/generations', poll: (id: string) => `/v1/video/generations/${id}` };

/**
 * 根据Model端点类型查找对应的Submit/轮询 URL 路径
 */
 ...
 * 优先使用 Gemini /api/pricing_new 同步的元数据，fallback 到Model名推断
 */
...
function handleVideoSubmitError(
  ...
  let errorMessage = `Video API Error: ${status}`;
  try {
    const errorJson = JSON.parse(errorText);
    errorMessage = errorJson.error?.message || errorJson.message || errorMessage;
  } catch { /* ignore */ }
  if (status === 401 || status === 403) throw new Error('API Key None效或已过期');
  if (status === 429) {
    const err = new Error('API 请求过于频繁，请稍后重试') as Error & { status?: number };
    err.status = 429;
    throw err;
  }
  // 所有 500/502/503/529 均视为可重试的临时服务Error，携带 status 供重试机制识别
  if (status >= 500) {
...
/**
 * Video Generation API 通常要求输入Image满足最小尺寸（如 Seedance 要求宽度 ≥ 300px）。
 * 当九宫格切割后的Image尺寸过小时，自动放大到满足最低要求后重新Upload。
 * @param imageUrl  HTTP URL Image地址
 * @param minDimension  宽高的最小像素值（默认 300，匹配 Seedance 等Model要求）
 * @returns 原始 URL（尺寸达标）或放大后重新Upload的新 URL
 */
async function ensureMinImageSize(
  ...
  try {
    // 通过 fetch 加载Image为 blob，避免 CORS 问题
    ...
    URL.revokeObjectURL(objectUrl); // drawImage Done后释放
    objectUrl = undefined;
...
/** AbortSignal 感知的 sleep：若信号触Hair则立即以 '用户已Cancel' 拒绝 */
function sleepOrAbort(ms: number, signal?: AbortSignal): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    if (signal?.aborted) return reject(new Error('用户已Cancel'));
    const tid = setTimeout(resolve, ms);
    signal?.addEventListener('abort', () => { clearTimeout(tid); reject(new Error('用户已Cancel')); }, { once: true });
  });
}

// Call video generation API — 根据Model自动路由到正确的 Gemini API 格式
export async function callVideoGenerationApi(
  ...
  /** Seedance 2.0: Video引用 URL 列表 (运镜/动作复刻) */
  videoRefs?: string[],
  /** Seedance 2.0: Audio引用 URL 列表 (节奏/BGM) */
  audioRefs?: string[],
  /** Seedance 2.0: 是否生成Audio（默认 true） */
  enableAudio?: boolean,
  /** Seedance 2.0: 是否锁定运镜（默认 false） */
  cameraFixed?: boolean,
  /** 外部中止信号，用于Stop Generation时真正Cancel网络请求 */
  signal?: AbortSignal,
): Promise<string> {
  const featureConfig = getFeatureConfig('video_generation');
  const resolvedPlatform = platform || featureConfig?.platform;
  if (!resolvedPlatform) {
    throw new Error('Please first在设置中配置Video Generation服务映射');
  }
  const model = featureConfig?.models?.[0];
  if (!model) {
    throw new Error('Please first在设置中配置Video GenerationModel');
  }
  const videoBaseUrl = featureConfig?.baseUrl?.replace(/\/+$/, '');
  if (!videoBaseUrl) {
    throw new Error('Please first在设置中配置Video Generation服务映射');
  ...
  // 确保所有输入Image满足Video API 的最小尺寸要求（如 Seedance ≥ 300px）
  ...
  return retryOperation(() => {
    if (signal?.aborted) return Promise.reject(new Error('用户已Cancel'));
...
// Gemini 文档: POST /v1/video/generations (primary) + /v1/video/create (fallback)
...
async function callUnifiedVideoApi(
  ...
  if (!taskId) {
    ...
    throw new Error(`Back空的任务 ID（响应格式未识别，请检查控制台日志）`);
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (status === 'completed' || status === 'succeeded' || status === 'success') {
      ...
      if (!videoUrl) throw new Error('任务Done但没有Video URL');
      return videoUrl;
    }

    if (status === 'failed' || status === 'error' || status === 'cancelled') {
      const errorMsg = statusData.error?.message || statusData.error || statusData.message || 'VideoGeneration failed';
      throw new Error(typeof errorMsg === 'string' ? errorMsg : JSON.stringify(errorMsg));
    }
  }
  throw new Error('Video Generation超时');
}

// ==================== Volcengine 豆包/Seedance 格式 ====================
// Gemini 文档: POST /volc/v1/contents/generations/tasks + GET /volc/v1/contents/generations/tasks/{taskId}
...
async function callVolcVideoApi(
  ...
  onProgress?: (progress: number) => void,
  keyManager?: { handleError: (status: number, errorText?: string) => boolean },
  /** Seedance 2.0: Video引用 URL 列表 */
  videoRefs?: string[],
  /** Seedance 2.0: Audio引用 URL 列表 */
  audioRefs?: string[],
  signal?: AbortSignal,
): Promise<string> {
  ...
  // Image内容（First Frame/Tail Frame）
  ...
  // Seedance 2.0 多模态：Video引用（延长/Edit/运镜复刻等）
  ...
  // Seedance 2.0 多模态：Audio引用（BGM/卡点等）
  ...
  // 检测代理包装的业务级Error（HTTP 200 但 body.status 为 failed/error）
  // 典型Scene：Gemini 中转将上游 451（内容审核）等Error包装为 {status: "failed", message: "..."}
  if (submitData.status === 'failed' || submitData.status === 'error') {
    const proxyMsg = submitData.message || submitData.error?.message || 'VideoSubmitFailed（代理Back业务Error）';
    console.error('[VideoGen] Volc: proxy-wrapped business error:', proxyMsg);
    // 尝试从Error信息中提取原始 HTTP 状态码
  ...
  // 提取任务 ID（兼容多种响应格式）
  // Gemini 中转: { id: "cgt-..." }  /  原生火山方舟: { id: "01973..." }
  ...
  if (!taskId) {
    ...
    // 兜底：将代理Back的Error信息（如有）附加到异常中，避免信息丢失
    const detail = submitData.message || submitData.error?.message || '';
    throw new Error(detail || `doubao-seedance Back空的任务 ID（响应格式未识别，请检查控制台日志）`);
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (status === 'succeeded') {
      // 兼容多种响应格式提取Video URL
      const videoUrl =
        normalizeUrl(statusData.content?.video_url) ||      // Gemini 中转格式
      ...
      if (!videoUrl) {
        ...
        throw new Error('任务Done但没有Video URL');
      }
      return videoUrl;
    }

    if (status === 'failed' || status === 'expired' || status === 'cancelled') {
      const errorMsg = statusData.error?.message || statusData.error?.code || 'VideoGeneration failed';
      throw new Error(typeof errorMsg === 'string' ? errorMsg : JSON.stringify(errorMsg));
    }
...
// ==================== 通义万象 wan 格式 ====================
// Gemini 文档:
//   Create: POST /alibailian/api/v1/services/aigc/video-generation/video-synthesis
...
async function callWanVideoApi(
  ...
  if (!taskId) throw new Error('Back空的任务 ID');
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (taskStatus === 'FAILED') {
      throw new Error(statusData.output?.message || statusData.output?.error || 'VideoGeneration failed');
    }
...
// ==================== Kling 可灵全系列格式 ====================
// Gemini: POST /kling/v1/videos/{path} + GET /kling/v1/videos/{path}/{task_id}

/**
 * Resolve kling model name for API requests.
 * Composite IDs like 'kling-image-v1-5' → 'kling-v1-5' (Gemini version ID).
...
async function callKlingVideoApi(
  ...
  // 轮询 URL 镜像Submit路径: GET /kling/v1/videos/{path}/{task_id}
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (taskStatus === 'failed' || taskStatus === 'error') {
      throw new Error(statusData.data?.task_status_msg || statusData.message || 'VideoGeneration failed');
    }
  }
  throw new Error('Video Generation超时');
}
...
// Gemini: POST /v1/videos (FormData) + GET /v1/videos/{taskId}
...
async function callOpenAIOfficialVideoApi(
  ...
  const directUrl = extractVideoUrl(submitData);
  if (directUrl) return directUrl;
  if (!taskId) throw new Error('Sora Back空任务 ID');
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (status === 'completed' || status === 'succeeded' || status === 'success') {
      ...
      if (!videoUrl) throw new Error('Sora 任务Done但没有Video URL');
      return videoUrl;
    }

    if (status === 'failed' || status === 'error') {
      throw new Error(statusData.error?.message || statusData.error || statusData.message || 'Sora Generation failed');
    }
  }
...
// ==================== Replicate Video格式 ====================
// Gemini: POST /replicate/v1/predictions + GET /replicate/v1/predictions/{id}
...
async function callReplicateVideoApi(
  ...
  if (!predictionId) throw new Error('Replicate Back空 prediction ID');
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (status === 'succeeded') {
      const videoUrl = extractVideoUrl(statusData);
      if (!videoUrl) throw new Error('Replicate Success但未BackVideo URL');
      return videoUrl;
    }

    if (status === 'failed' || status === 'canceled') {
      throw new Error(statusData.error || 'Replicate VideoGeneration failed');
    }
  }
  throw new Error('Replicate Video Generation超时');
}
...
export async function extractLastFrameFromVideo(
  ...
  // local-image:// 是 Electron 注册的Custom协议，可以直接使用
  // 不需要转换为 file://
  ...
  return new Promise((resolve) => {
    ...
    // 方法：使用 timeupdate 监听播放Progress，当接近目标时间时捕获
    video.ontimeupdate = () => {
    ...
    video.onseeked = () => {
      ...
      if (Math.abs(video.currentTime - targetTime) < 0.5) {
        // seek Success，等待一下再捕获
        setTimeout(captureFrame, 200);
      } else {
        // seek 可能Failed，尝试播放到目标位置
...
export async function callJuxinVideoGenerationApi(
  ...
  if (!taskId) {
    throw new Error('Grok API Back空的任务 ID');
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (status === 'failed' || status === 'error') {
      const errorMsg = statusData.error || statusData.error_message || 'VideoGeneration failed';
      throw new Error(typeof errorMsg === 'string' ? errorMsg : JSON.stringify(errorMsg));
    }
