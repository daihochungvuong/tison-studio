...
export function ExportView() {
  ...
  // === Progress计算：合并 Script shots 和 Director splitScenes 的状态 ===
  ...
  const handleExportToFolder = useCallback(async () => {
    ...
    try {
      if (hasSplitScenes) {
        ...
        if (success) toast.success('ExportDone！');
      } else if (scriptData) {
        ...
        toast.error('没Has 可Export的数据');
      }
    } catch (error) {
      toast.error(`ExportFailed: ${(error as Error).message}`);
    } finally {
  ...
  const handleDownloadFiles = useCallback(async () => {
    ...
    try {
      ...
      toast.success('DownloadDone！');
    } catch (error) {
      toast.error(`DownloadFailed: ${(error as Error).message}`);
    } finally {
      setIsExporting(false);
      setExportProgress(null);
    }
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="bg-card border border-border rounded-xl p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="mb-10">
                ...
                {hasSplitScenes && (
                  <div className="flex items-center gap-4 mt-2 text-[10px] text-muted-foreground">
                    <span>Image: {imageReadyItems}/{totalItems}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  {isExporting ? (
                    ...
                    <><FolderOpen className="w-4 h-4 mr-2" />SelectFolderExport</>
                  )}
                </Button>
              ...
              {hasSplitScenes && directorStats && (
                <div className="mt-4 text-xs text-muted-foreground">
                  可Export: {directorStats.imagesReady} 张First Frame · {directorStats.videosReady} Video{directorStats.endFramesReady > 0 ? ` · ${directorStats.endFramesReady} 张Tail Frame` : ''}
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground mb-1">素材Download</h4>
                  <p className="text-[10px] text-muted-foreground">
                    Download所Has Generated的Image和Video素材
                  </p>
                </div>
              </div>
