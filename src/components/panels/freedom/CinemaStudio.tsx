...
export function CinemaStudio() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      toast.success('电影级ImageGenerated successfully！Saved到Media Library');
    } catch (err: any) {
    ...
    <div className="flex h-full">
      ...
      <div className="w-[420px] border-r flex flex-col">
        ...
        <div className="p-4 border-t space-y-3">
          <Textarea
            placeholder="Description你的电影Scene..."
            value={cinemaPrompt}
          ...
          {cinemaPrompt.trim() && (
            <details className="text-xs">
              <summary className="text-muted-foreground cursor-pointer">View compiled prompt</summary>
          ...
          >
            {cinemaGenerating ? (
              ...
              <><Sparkles className="mr-2 h-4 w-4" /> Cinematic Shot</>
            )}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-muted/30">
        {cinemaGenerating ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground">Generating cinematic image......</p>
          </div>
        ) : cinemaResult ? (
          ...
          <div className="flex flex-col items-center gap-3 text-muted-foreground">
            ...
            <p className="text-lg font-medium">Cinema Studio</p>
            <p className="text-sm">Select相机、Shot、Focal Length、Aperture，Generate电影级Image</p>
            <p className="text-xs text-muted-foreground/60">Camera parameters auto-compile to AI config</p>
          </div>
        )}
      </div>
